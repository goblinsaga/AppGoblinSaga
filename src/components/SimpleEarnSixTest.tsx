...
import WGsaTokenBalanceFour from './WGSABalance5';

// ABI del contrato MATICXGSAst actualizado
...
const contractAddress = "0x5A0b55E1dbD455d3973fA3A2B807AB156b91a9B7"; // Nueva dirección del contrato
const maticXAddress = "0xfa68FB4628DFF1028CFEc22b4162FCcd0d45efb6"; // Dirección de MaticX

const SEHeaderSix: React.FC = () => {
    ...
    const fetchWalletBalance = async () => {
        try {
            const maticXContract = new ethers.Contract(maticXAddress, ["function balanceOf(address) view returns (uint256)"], provider?.getSigner());
            const balance = await maticXContract.balanceOf(address);
            if (balance) {
                const formattedBalance = ethers.utils.formatUnits(balance, 18); // MaticX tiene 18 decimales
        ...
        try {
            ...
            if (stakeInfo) {
                const formattedStakeInfo = {
                    amountStaked: ethers.utils.formatUnits(stakeInfo.amountStaked, 18), // MaticX tiene 18 decimales
    ...
    const handleApprove = async () => {
        ...
        try {
            const maticXContract = new ethers.Contract(maticXAddress, [
                "function approve(address spender, uint256 amount) returns (bool)"
            ], provider.getSigner());

            const amount = ethers.utils.parseUnits(amountToStake.toString(), 18); // MaticX tiene 18 decimales
            const tx = await maticXContract.approve(contractAddress, amount);
            await tx.wait();
            setSuccessMessage("Approval successful!");
        } catch (error) {
            console.error("Error approving MaticX:", error);
            setErrorMessage("Error approving MaticX. Try Again.");
        }
    };
    ...
    const handleStake = async () => {
        ...
        try {
            ...
            const maticXContract = new ethers.Contract(
                maticXAddress,
            ...
            const allowance = await maticXContract.allowance(address, contractAddress);
            const amountToStakeInWei = ethers.utils.parseUnits(amountToStake.toString(), 18); // MaticX tiene 18 decimales

            if (allowance.lt(amountToStakeInWei)) {
                const approveTx = await maticXContract.approve(contractAddress, amountToStakeInWei);
    ...
    const handleWithdraw = async (amount: string) => {
        ...
        try {
            ...
            const withdrawTx = await contract.withdraw(ethers.utils.parseUnits(amount, 18)); // MaticX tiene 18 decimales
            await withdrawTx.wait();
            fetchStakeInfo();
            fetchRewards();
            setSuccessMessage("Withdrawal successful!");
        } catch (error) {
            console.error("Error withdrawing MaticX:", error);
            setErrorMessage("Error withdrawing MaticX. Try Again.");
        } finally {
            ...
            <div style={{ height: "auto", marginBottom: "20px" }} className="blog__item">
                ...
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", textAlign: "center" }}>
                    <a className="metaportal_fn_buttonLW" href='https://www.staderlabs.com/polygon/stake/' target='blank' style={{ textDecoration: "none" }}>
                        <p style={{ fontSize: "15px", color: "white" }}>Get MATICX</p>
                    </a>
                </div>
            </div>
            <div id="MATICX-Staking">
                <div className="fn_cs_news">
                    <div className="news_part">
                        <div className="left_items">
                            <div className="blog__item">
                                <div className="meta">
                                    <p>Stake MATICX Earn WGSA</p>
                                </div>
                                <div className="title">
                                    <h3>MATICX/WGSA Stake</h3>
                                </div>
                                <div
                                    className="image"
                                    style={{
                        ...
                        <div className="right_items">
                            ...
                            <div className="blog__item">
                                <div className="counter">
                                    <span className="cc">
                                        <img style={{ width: "90%", marginTop: "-3px" }} src="/img/maticx.png" alt="MaticX" />
                                    </span>
                                </div>
                                <div className="meta">
                                    <p>MATICX Staked</p>
                            ...
                            <div className="blog__item">
                                ...
                                <div className="containerGrid">
                                    ...
                                    <a style={{ textDecoration: "none" }} href='https://docs.goblinsaga.xyz/features/maticx-gsa-staking' target='blank'>
